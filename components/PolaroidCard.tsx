...
const ShimmeringLoader = () => (
    <div className="w-full h-full bg-neutral-800 relative overflow-hidden">
        <div 
            className="absolute inset-0 -translate-x-full animate-[shimmer_2s_infinite] bg-[linear-gradient(90deg,transparent,rgba(255,255,255,0.08),transparent)]"
        />
    </div>
);
...
const ErrorDisplay = ({ onRetry }: ErrorDisplayProps) => (
    <div className="flex flex-col items-center justify-center h-full text-center p-4 text-neutral-400">
        ...
        <p className="font-semibold text-sm mb-3">Generation Failed</p>
        {onRetry && (
            <button
                onClick={(e) => {
                ...
                className="font-permanent-marker text-sm text-center text-white bg-red-500 py-1 px-4 rounded-sm transform transition-transform duration-200 hover:scale-105 hover:bg-red-600 shadow-[1px_1px_0px_1px_rgba(0,0,0,0.1)]"
            >
                Retry
            </button>
        )}
    ...
    const cardInnerContent = (
        <>
            <div className="w-full bg-neutral-900 shadow-inner flex-grow relative overflow-hidden group">
                {status === 'pending' && <ShimmeringLoader />}
                {status === 'error' && <ErrorDisplay onRetry={onShake ? () => onShake(caption) : undefined} />}
